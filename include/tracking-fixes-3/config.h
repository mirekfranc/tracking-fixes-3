				...
				for (std::string line; getline(file, line);) {
					...
					if (general) {
						if (key == "ignored-branches")
							split_string(this->ignored_branches, list, ',');
					} else if (stable_branches) {
						std::vector<std::string> branches;
						split_string(branches, list, ',');
						this->stable_branches.insert(std::make_pair(key, std::move(branches)));
						continue;
					}
				}
			}

		operator bool() const { return !this->ignored_branches.empty() && !this->stable_branches.empty(); }
	};
